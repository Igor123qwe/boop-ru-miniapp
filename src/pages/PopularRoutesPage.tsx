...
export const PopularRoutesPage: React.FC<Props> = ({ city, onBack, initialRouteId }) => {
  ...
  const fetchPhotosFromBackend = async (params: URLSearchParams): Promise<string[]> => {
    const photosUrls = [
  ...
  const parsePhotosFromBackend = async (params: URLSearchParams): Promise<string[]> => {
    const parsePayload = {
      ...
      {zoomedImage && (
        <div className="image-zoom-backdrop" onClick={closeZoomImage}>
          <div className="image-zoom-modal" onClick={e => e.stopPropagation()}>
            ...
            <img src={zoomedImage} alt={zoomedImageTitle} className="image-zoom-img" />
          </div>
        </div>
      )}
    </div>
